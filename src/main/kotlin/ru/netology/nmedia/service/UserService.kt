...
class UserService(
    ...
    fun welcome(id:Long) {
        val userPushTokenList = pushTokenRepository.findByUserId(id).ifEmpty { return }
        if (id == 0L) {
            val message = "Hello, Anonymous"
            userPushTokenList.forEach { tokenAnon ->
                pushService.send(tokenAnon.token, PushMessage(id,message))
            }
        } else {
            val user = userRepository.findByIdOrNull(id)
            val message = "Hello, ${user?.name}"
            pushService.send(userPushTokenList.last().token, PushMessage(id,message))
        }
    }
